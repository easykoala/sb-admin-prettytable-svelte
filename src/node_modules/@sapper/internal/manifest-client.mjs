// This file is generated by Sapper — do not edit it!
export { default as Root } from '../../../routes/_layout.svelte';
export { preload as root_preload } from './shared';
export { default as ErrorComponent } from '../../../routes/_error.svelte';

export const ignore = [/^\/pretty\/store\/?$/, /^\/blog\.json$/, /^\/blog\/([^\/]+?)\.json$/];

export const components = [
	{
		js: () => import("../../../routes/index.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:index.svelte__"
	},
	{
		js: () => import("../../../routes/activity_log.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:activity_log.svelte__"
	},
	{
		js: () => import("../../../routes/settings.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:settings.svelte__"
	},
	{
		js: () => import("../../../routes/layouts/static_navigation.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:layouts/static_navigation.svelte__"
	},
	{
		js: () => import("../../../routes/layouts/light_sidenav.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:layouts/light_sidenav.svelte__"
	},
	{
		js: () => import("../../../routes/charts.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:charts.svelte__"
	},
	{
		js: () => import("../../../routes/pretty/table_edit.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:pretty/table_edit.svelte__"
	},
	{
		js: () => import("../../../routes/pretty/table.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:pretty/table.svelte__"
	},
	{
		js: () => import("../../../routes/tables.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:tables.svelte__"
	},
	{
		js: () => import("../../../routes/pages/_layout.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:pages/_layout.svelte__"
	},
	{
		js: () => import("../../../routes/pages/authentication/_layout.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:pages/authentication/_layout.svelte__"
	},
	{
		js: () => import("../../../routes/pages/authentication/forget_password.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:pages/authentication/forget_password.svelte__"
	},
	{
		js: () => import("../../../routes/pages/authentication/register.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:pages/authentication/register.svelte__"
	},
	{
		js: () => import("../../../routes/pages/authentication/login.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:pages/authentication/login.svelte__"
	},
	{
		js: () => import("../../../routes/pages/error/_layout.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:pages/error/_layout.svelte__"
	},
	{
		js: () => import("../../../routes/pages/error/error_401.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:pages/error/error_401.svelte__"
	},
	{
		js: () => import("../../../routes/pages/error/error_404.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:pages/error/error_404.svelte__"
	},
	{
		js: () => import("../../../routes/pages/error/error_500.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:pages/error/error_500.svelte__"
	},
	{
		js: () => import("../../../routes/blog/index.svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:blog/index.svelte__"
	},
	{
		js: () => import("../../../routes/blog/[slug].svelte"),
		css: "__SAPPER_CSS_PLACEHOLDER:blog/[slug].svelte__"
	}
];

export const routes = (d => [
	{
		// index.svelte
		pattern: /^\/$/,
		parts: [
			{ i: 0 }
		]
	},

	{
		// activity_log.svelte
		pattern: /^\/activity_log\/?$/,
		parts: [
			{ i: 1 }
		]
	},

	{
		// settings.svelte
		pattern: /^\/settings\/?$/,
		parts: [
			{ i: 2 }
		]
	},

	{
		// layouts/static_navigation.svelte
		pattern: /^\/layouts\/static_navigation\/?$/,
		parts: [
			null,
			{ i: 3 }
		]
	},

	{
		// layouts/light_sidenav.svelte
		pattern: /^\/layouts\/light_sidenav\/?$/,
		parts: [
			null,
			{ i: 4 }
		]
	},

	{
		// charts.svelte
		pattern: /^\/charts\/?$/,
		parts: [
			{ i: 5 }
		]
	},

	{
		// pretty/table_edit.svelte
		pattern: /^\/pretty\/table_edit\/?$/,
		parts: [
			null,
			{ i: 6 }
		]
	},

	{
		// pretty/table.svelte
		pattern: /^\/pretty\/table\/?$/,
		parts: [
			null,
			{ i: 7 }
		]
	},

	{
		// tables.svelte
		pattern: /^\/tables\/?$/,
		parts: [
			{ i: 8 }
		]
	},

	{
		// pages/authentication/forget_password.svelte
		pattern: /^\/pages\/authentication\/forget_password\/?$/,
		parts: [
			{ i: 9 },
			{ i: 10 },
			{ i: 11 }
		]
	},

	{
		// pages/authentication/register.svelte
		pattern: /^\/pages\/authentication\/register\/?$/,
		parts: [
			{ i: 9 },
			{ i: 10 },
			{ i: 12 }
		]
	},

	{
		// pages/authentication/login.svelte
		pattern: /^\/pages\/authentication\/login\/?$/,
		parts: [
			{ i: 9 },
			{ i: 10 },
			{ i: 13 }
		]
	},

	{
		// pages/error/error_401.svelte
		pattern: /^\/pages\/error\/error_401\/?$/,
		parts: [
			{ i: 9 },
			{ i: 14 },
			{ i: 15 }
		]
	},

	{
		// pages/error/error_404.svelte
		pattern: /^\/pages\/error\/error_404\/?$/,
		parts: [
			{ i: 9 },
			{ i: 14 },
			{ i: 16 }
		]
	},

	{
		// pages/error/error_500.svelte
		pattern: /^\/pages\/error\/error_500\/?$/,
		parts: [
			{ i: 9 },
			{ i: 14 },
			{ i: 17 }
		]
	},

	{
		// blog/index.svelte
		pattern: /^\/blog\/?$/,
		parts: [
			{ i: 18 }
		]
	},

	{
		// blog/[slug].svelte
		pattern: /^\/blog\/([^\/]+?)\/?$/,
		parts: [
			null,
			{ i: 19, params: match => ({ slug: d(match[1]) }) }
		]
	}
])(decodeURIComponent);

if (typeof window !== 'undefined') {
	import("G:/svelte/sb-admin-prettytable-svelte/node_modules/sapper/sapper-dev-client.js").then(client => {
		client.connect(10000);
	});
}